...
const base = { public_sku: "PUBLIC-1", approved_title: "Approved title", public_price: "", price_mode: "request_quote", pricing_approved: "false", checkout_active: "false", shipping_class: "", taxable: "false", stripe_price_id: "", allowed_countries: "", stripe_shipping_rate_id: "", automatic_tax: "false", notes: "" };
...
test("rejects incomplete price, shipping, and tax activation", () => {
  assert.throws(() => validatePricingRows([{ ...base, checkout_active: "true" }], catalog), /approval/);
  assert.throws(() => validatePricingRows([{ ...base, pricing_approved: "true", price_mode: "fixed" }], catalog), /positive/);
  assert.throws(() => validatePricingRows([{ ...base, pricing_approved: "true", price_mode: "fixed", public_price: "10", checkout_active: "true" }], catalog), /shipping/);
  const shipping = { ...base, pricing_approved: "true", price_mode: "fixed", checkout_active: "true", public_price: "10", shipping_class: "parcel", allowed_countries: "US", stripe_shipping_rate_id: "shr_test", taxable: "true" };
...
test("allows an approved fixed price while checkout remains disabled", () => {
  const row = validatePricingRows([{ ...base, public_price: "10", price_mode: "fixed", pricing_approved: "true" }], catalog)[0];
  assert.equal(row.public_price, 10);
