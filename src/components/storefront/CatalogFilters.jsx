...
export default function CatalogFilters({ query, category, manufacturer, availability, sort, categories, manufacturers, onQuery, onCategory, onManufacturer, onAvailability, onSort, onReset }) {
  ...
  return (
    <div className="ts-filter-panel" aria-label="Catalog filters">
      <div className="ts-filter-fields" tabIndex="0" aria-label="Scroll for all catalog filters">
        ...
        <label><span>Availability</span><select value={availability} onChange={(event) => onAvailability(event.target.value)}><option value="All">All quote options</option><option value="fixed">Listed merchandise price</option><option value="request_quote">Request quote</option></select></label>
        ...
        <button type="button" className="ts-filter-reset" onClick={onReset} disabled={!active}>Reset</button>
      </div>
    </div>
  );
}
