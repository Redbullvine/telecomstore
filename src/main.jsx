...
// Counts only — generated by scripts/build-general-merchandise-catalog.mjs so the
...
function StorefrontHeader({ query, setQuery, onSubmit, onClear, cartCount, navigate, onQuote, onCategory }) {
  ...
  return <header className="ts-head ts-retail-head">
    ...
    {menuOpen ? <nav className="ts-mobile-store-menu" id="mobile-store-menu" aria-label="Mobile store menu"><button type="button" onClick={() => { navigate("/shop"); setMenuOpen(false); }}>General Merchandise Shop</button><button type="button" onClick={() => { navigate("/custom-workwear"); setMenuOpen(false); }}>Custom Workwear</button>{CATALOG_CATEGORIES.map((item) => <button key={item.name} type="button" onClick={() => { onCategory(item.name); setMenuOpen(false); }}>{item.name}</button>)}<button type="button" onClick={() => { onQuote(); setMenuOpen(false); }}>Request a Quote</button></nav> : null}
  </header>;
}
...
function RetailHomepage({ products, navigate, navigateCategory, navigateProduct, isQuoted, onAddQuote, onAsk, onQuote }) {
  return <div className="ts-retail-home">
    ...
    <section className="ts-department-section ts-marketplace-section"><div className="ts-wrap"><div className="ts-retail-section-head"><div><p>General Merchandise Shop</p><h2>{MARKETPLACE_SUMMARY.product_count.toLocaleString()} everyday products, priced and ready</h2></div><a href="/shop" onClick={(event) => { event.preventDefault(); navigate("/shop"); }}>Shop all departments</a></div><div className="ts-department-grid">
      {MARKETPLACE_SUMMARY.departments.map((department) => <a className="ts-department-tile" key={department.slug} href={`/shop/${department.slug}`} onClick={(event) => { event.preventDefault(); navigate(`/shop/${department.slug}`); }}><CategoryIcon category={department.name} size={38} /><span><strong>{department.name}</strong><small>{department.count.toLocaleString()} products</small></span></a>)}
