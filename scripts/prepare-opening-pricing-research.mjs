...
for (const product of catalog) {
  ...
  researched.push({ public_sku: product.sku, manufacturer_mpn: mpn, manufacturer: product.brand, category: product.category, title: product.title, lowest_reliable_price: round(low), typical_public_price: round(typical), highest_reasonable_price: round(high), proposed_retail_price: proposed, pricing_evidence_urls: evidenceUrls, evidence_date: "2026-08-03", confidence, pricing_status: status, notes: reason });
  publicReview.push({ public_sku: product.sku, manufacturer_mpn: mpn, manufacturer: product.brand, title: product.title, pricing_status: status, confidence, reason, proposed_retail_price: proposed, evidence_urls: evidenceUrls });
  ...
  template.push({ public_sku: product.sku, approved_title: product.title, public_price: proposed ?? "", checkout_active: "false", shipping_class: assessment.shipping_class, taxable: "false", stripe_price_id: "", allowed_countries: "", stripe_shipping_rate_id: "", automatic_tax: "false", notes: `${status}; Danny approval required; checkout remains disabled` });
}
